...
public class GitlabClient extends BaseClient {
	...
	public GitlabClient(final GitlabNotifierConfig config, final Logger logger) {
		...
		getRequestBuilder().addHeader(TOKEN, config.getToken());
	}

	public List<GitlabIssue> getIssues() throws IOException {
		final Request request = getRequestBuilder().url(String.format("%s/projects/%s/issues?labels=%s&state=opened",
				config.getUrl(), config.getProjectId(), config.getLabel())).build();
		...
		if (statusCode != HttpURLConnection.HTTP_OK || Objects.isNull(response.body())) {
			throw new IllegalArgumentException(
					String.format("Could not retrieve GitLab issues: %s, %d", config.getUrl(), statusCode));
		}
	...
	public GitlabIssue createIssue(final GitlabIssue issue) throws IOException {
		...
		final Request request = getRequestBuilder()
				.url(String.format("%s/projects/%s/issues", config.getUrl(), config.getProjectId())).post(requestBody)
				.build();
		final Response response = getClient().newCall(request).execute();
		final int statusCode = response.code();
		if ((statusCode != HttpURLConnection.HTTP_CREATED && statusCode != HttpURLConnection.HTTP_OK)
			...
			throw new IllegalArgumentException(
					String.format("Could not create GitLab issue: %s, %d", config.getUrl(), statusCode));
		}
